...
const Secretaries: React.FC = () => {
  ...
  const handleAddSecretary = async () => {
    try {
      ...
      setSuccess('تم إضافة السكرتير بنجاح');
      fetchSecretaries();
    } catch (error: any) {
      setError(error.response?.data?.message || 'خطأ في إضافة السكرتير');
    }
  };
  ...
  const handleDeleteSecretary = async (id: string) => {
    if (window.confirm('هل أنت متأكد من حذف هذا السكرتير؟')) {
      try {
        ...
        setSuccess('تم حذف السكرتير بنجاح');
        fetchSecretaries();
      } catch (error: any) {
            ...
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                ...
                <Box>
                  ...
                  <Typography variant="h4">
                    {secretaries.reduce((sum, s) => sum + s.totalEarnings, 0).toLocaleString()} ريال
                  </Typography>
                </Box>
              </Box>
            </CardContent>
            ...
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                ...
                <Box>
                  ...
                  <Typography variant="h4">
                    {secretaries.reduce((sum, s) => sum + s.totalDebt, 0).toLocaleString()} ريال
                  </Typography>
                </Box>
              </Box>
            </CardContent>
