...
const SecretaryDetail: React.FC = () => {
  ...
  const fetchSecretaryDetails = async () => {
    try {
      ...
      console.error('خطأ في جلب تفاصيل السكرتير:', error);
    }
  };
  ...
  const fetchSecretaryVisas = async () => {
    try {
      ...
      console.error('خطأ في جلب تأشيرات السكرتير:', error);
      setLoading(false);
    }
  };
  ...
  if (!secretary) {
    return (
      <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
        <Typography>لم يتم العثور على السكرتير</Typography>
      </Container>
    );
  }
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>أرباح السكرتير</TableCell>
              ...
              <TableBody>
                {visas.map((visa) => (
                  <TableRow key={visa._id}>
                    ...
                    <TableCell>{visa.totalExpenses.toLocaleString()} ريال</TableCell>
                    <TableCell>
                      {visa.sellingPrice ? `${visa.sellingPrice.toLocaleString()} ريال` : '-'}
                    </TableCell>
                    <TableCell>
                      {visa.profit ? `${visa.profit.toLocaleString()} ريال` : '-'}
                    </TableCell>
                    <TableCell>
                      {visa.secretaryEarnings ? `${visa.secretaryEarnings.toLocaleString()} ريال` : '-'}
                    </TableCell>
