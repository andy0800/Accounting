...
// الحصول على سكرتير محدد مع المعلومات التفصيلية
router.get('/:id', async (req, res) => {
  try {
    ...
    // الحصول على جميع التأشيرات لهذا السكرتير
...
router.post('/', async (req, res) => {
  try {
    ...
    // التحقق من وجود سكرتير بنفس الاسم
    const existingSecretary = await Secretary.findOne({ name });
    if (existingSecretary) {
      return res.status(400).json({ message: 'يوجد سكرتير بهذا الاسم بالفعل' });
    }
