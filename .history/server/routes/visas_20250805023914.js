...
router.post('/:id/expenses', async (req, res) => {
  try {
    ...
    // Validate input
    console.log('Received expense data:', { amount, description, stage, date });
    
    if (!amount || isNaN(parseFloat(amount)) || parseFloat(amount) <= 0) {
      console.log('Amount validation failed:', { amount, parsed: parseFloat(amount) });
    ...
    console.log('Stage validation:', { stage, validStages, includes: validStages.includes(stage) });
    if (!stage || !validStages.includes(stage)) {
      console.log('Stage validation failed:', { stage, validStages });
