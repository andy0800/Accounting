...
router.get('/visas/:status', async (req, res) => {
  try {
    ...
    worksheet.columns = [
      ...
      { header: 'السكرتير', key: 'secretary', width: 20 },
      ...
      { header: 'أرباح السكرتير', key: 'secretaryEarnings', width: 20 },
    ...
    visas.forEach(visa => {
      ...
      worksheet.addRow({
        reference,
        name: visa.name,
        dateOfBirth: moment(visa.dateOfBirth).format('DD/MM/YYYY'),
        nationality: visa.nationality,
        passportNumber: visa.passportNumber,
        visaNumber: visa.visaNumber,
        secretary: visa.secretary.name,
        currentStage: visa.currentStage,
        visaIssueDate: moment(visa.visaIssueDate).format('DD/MM/YYYY'),
        visaExpiryDate: moment(visa.visaExpiryDate).format('DD/MM/YYYY'),
        visaDeadline: moment(visa.visaDeadline).format('DD/MM/YYYY'),
        ...
        createdAt: moment(visa.createdAt).format('DD/MM/YYYY'),
        status: visa.status
      });
    });
    ...
    res.setHeader('Content-Disposition', `attachment; filename=تأشيرات-${status}-${moment().format('YYYY-MM-DD')}.xlsx`);
...
router.get('/secretary/:id', async (req, res) => {
  try {
    ...
    // إضافة قسم الملخص
    worksheet.addRow(['تقرير السكرتير']);
    worksheet.addRow(['']);
    ...
    worksheet.addRow([
      ...
      'سعر البيع', 'الربح', 'أرباح السكرتير', 'اسم العميل',
    ...
    // تعيين رؤوس الاستجابة
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename=سكرتير-${secretary.code}-${moment().format('YYYY-MM-DD')}.xlsx`);
...
router.get('/company-report', async (req, res) => {
  try {
    ...
    // تعريف الأعمدة
    worksheet.addRow([
      'المرجع', 'الاسم', 'السكرتير', 'الحالة', 'إجمالي المصروفات',
      'سعر البيع', 'الربح', 'أرباح السكرتير', 'ربح الشركة',
...
router.get('/expenses/:visaId', async (req, res) => {
  try {
    ...
    worksheet.addRow(['السكرتير:', visa.secretary.name]);
