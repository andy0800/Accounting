...
const FarwaniyaInvoicesBase: React.FC<Props> = ({ systemKey, title, basePath }) => {
  ...
  const fetchInvoices = useCallback(async () => {
    try {
      ...
      const response = await apiClient.get(`/api/${systemKey}/invoices`, { params });
  ...
  }, [page, typeFilter, search, systemKey]);
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      await apiClient.delete(`/api/${systemKey}/invoices/${deleteDialog.invoice._id}`);
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await apiClient.get(`/api/exports/${systemKey}/invoices`, {
        params,
        responseType: 'blob',
      });
      ...
      link.href = url;
      link.setAttribute('download', `${systemKey}-invoices-${new Date().toISOString().split('T')[0]}.xlsx`);
    ...
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          ...
          <Button
            ...
            color="success"
            startIcon={<AddIcon />}
            onClick={() => navigate(`${basePath}/invoices/new?type=income`)}
          >
            فاتورة دخل
          </Button>
          <Button
            ...
            color="error"
            startIcon={<AddIcon />}
            onClick={() => navigate(`${basePath}/invoices/new?type=spending`)}
          >
            إيصال صرف
          </Button>
        </Box>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          ...
          <Button
            variant="text"
            onClick={() => navigate(`${basePath}/deleted`)}
          >
            عرض المحذوفات
          </Button>
        </Box>
                    ...
                    <TableRow key={invoice._id} hover>
                      ...
                      <TableCell>
                        <Tooltip title="عرض">
                          <IconButton
                            size="small"
                            onClick={() => navigate(`${basePath}/invoices/${invoice._id}`)}
                          >
                            <ViewIcon />
                          </IconButton>
                        </Tooltip>
                        <Tooltip title="تعديل">
                          <IconButton
                            size="small"
                            color="primary"
                            onClick={() => navigate(`${basePath}/invoices/${invoice._id}?edit=true`)}
                          >
                            <EditIcon />
                          </IconButton>
                        </Tooltip>
...
export default FarwaniyaInvoicesBase;
