...
const HSNewInvoice: React.FC = () => {
  ...
  useEffect(() => {
    const fetchFunding = async () => {
      try {
        const response = await apiClient.get('/api/home-service/dashboard');
        setFundingCredit(response.data.fundingCredit);
      } catch (err) {
        console.error('Error fetching funding credit:', err);
      }
    };
    fetchFunding();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Check funding credit for spending
    if (formData.type === 'spending' && parseFloat(formData.value) > fundingCredit) {
      setError(`رصيد التمويل غير كافٍ. المتاح: ${fundingCredit.toLocaleString('en-US', { minimumFractionDigits: 3 })} د.ك`);
      return;
    }
    ...
    try {
      ...
      await apiClient.post('/api/home-service/invoices', submitData);
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 800, mx: 'auto' }}>
      ...
      <Typography variant="h4" fontWeight="bold" sx={{ mb: 3 }}>
        {formData.type === 'income' ? 'فاتورة دخل جديدة' : 'إيصال صرف جديد'}
      </Typography>
